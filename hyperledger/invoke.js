...
// var firstnetwork_path = path.resolve('..', '..', 'sofana/sofana/construction-consortium-blockchain');
var org1tlscacert_path = path.resolve('hyperledger', 'crypto-config', 'peerOrganizations', 'cst.track.com', 'tlsca', 'tlsca.cst.track.com-cert.pem');
// console.log("org1tlscacert_path",org1tlscacert_path)
var org1tlscacert = fs.readFileSync(org1tlscacert_path, 'utf8');

// invoke();
module.exports ={
    invoke: async (data, callback)=> {
	   console.log('\n\n --- invoke.js - start');
	   try {
		   ...
		   const user = await fabric_client.getUserContext('by', true);
		   if (user && user.isEnrolled()) {
			   console.log('Successfully loaded "user1" from user store');
		   } else {
			   console.log('ailed to get user1.... run registerUser.js');
		   }
		...
		//    console.log("data.fnc",data.fnc);
		   ...
		   if (proposalResponses[0] instanceof Error) {
			...
			return callback({statusCode: 500, data: proposalResponses[0].toString()})
		   ...
		   if (results[1] instanceof Error) {
			   ...
			   callback({statusCode : 200,data : results})
			//    console.log('\n\n - try running "node query.js" to see the results');
			   ...
			   console.error(message);
			   callback({statusCode : 500,data : message })
			//    throw new Error(message);
		   }
	   } catch(error) {
		   console.log('Unable to invoke ::'+ error.toString());
		   callback({statusCode : 500,data : 'Unable to invoke ::'+ error.toString()})
	   }
	   console.log('\n\n --- invoke.js - end');
   }
